    ...
    while isOn:
        ...
        if player.player_collide_single(breakable_block):
            if player.shield_count > 0:
                ...
                is_exit = game_complete_screen(game_state="lose")
                if is_exit:
                    isOn = False
        if player.player_collide_single(unbreakable_block):
            if player.shield_count > 0:
                ...
                if is_exit:
                    isOn = False
        if player.score > 30:
            is_exit = game_complete_screen(game_state="win")
            if is_exit:
                isOn = False
...
def game_complete_screen(game_state="lose"):
    ...
    button_continue = game_mini_screen.ContinueButton(383, 258, lvl_button_continue, lvl_button_continue)
    button_exit = game_mini_screen.ExitButton(561, 258, lvl_button_exit, lvl_button_exit)
    button_replay = game_mini_screen.ReplayButton(472, 258, lvl_button_replay, lvl_button_replay)
    isOn = True
    while isOn:
        event_list = pygame.event.get()
        mouse_pos = pygame.mouse.get_pos()
        ...
        SCREEN.blit(esc_background, (200, 100))
        if game_state == "lose":
            display_lose = font.render("Game over", True, color)
            SCREEN.blit(display_lose, (370, 150))
        if game_state == "win":
            display_win = font.render("Level completed", True, color)
            SCREEN.blit(display_win, (310, 150))
